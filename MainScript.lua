...
local configPath = "Closet/Configs/"..game.PlaceId..".json"
makefolder("Closet")
makefolder("Closet/Configs")
...
local ClosetTitle = Instance.new("TextLabel",gui)
ClosetTitle.AnchorPoint = Vector2.new(0.5,0.5)
ClosetTitle.RichText = true
ClosetTitle.Text = '<font color="rgb(255,0,0)">C</font>loset '..GuiLibrary.Version
if GuiLibrary.CustomEdition ~= "" then
	ClosetTitle.Text = '<font color="rgb(255,0,0)">A</font>utumn '..GuiLibrary.Version..' ('..GuiLibrary.CustomEdition..' Edition)'
end
ClosetTitle.Size = UDim2.fromScale(0.07,0.024)
ClosetTitle.Position = UDim2.fromScale(0.05,0.05)
ClosetTitle.TextColor3 = Color3.fromRGB(255,255,255)
ClosetTitle.BackgroundTransparency = 1
ClosetTitle.TextSize = 16
ClosetTitle.TextXAlignment = Enum.TextXAlignment.Left
				...
				CreateToggle = function(tab2)
					...
					task.spawn(function()
						if config.Toggles[tab2["Name"]..tab["Name"]].Enabled then
							repeat task.wait() until shared.ClosetLoaded == true
							state.Enabled = true
							button.TextColor3 = (state.Enabled and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(255, 255, 255))
							if tab2["Function"] then
								tab2["Function"](state.Enabled)
...
Chatspammer = GuiLibrary.API.Windows.Player.CreateButton({
	...
	["Function"] = function(callback)
		if callback then
			task.spawn(function()
				repeat 
					game.TextChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync("ClosetV4 On Top")
					task.wait(7)
